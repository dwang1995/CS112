...
public class TrieNode {
    ...
    private static void checkKey(String key) {
        ...
        for(int i= 0; i< length; ++i){
            if((charToIndex(key.charAt(i))<0 || charToIndex(key.charAt(i))>25)){
    ...
    public boolean searchHelper(String key, TrieNode n) {
        checkKey(key);
        // TODO implement this method
        if(key.length()==0){
            return false;
        ...
        else if(key.length() == 1 && child.endsPath == true){
            return true;
        }
        else{
            return searchHelper(key.substring(1), child);
        }
    ...
    public void remove(String key) {
        checkKey(key);
        removeHelper(key,this);
        return;
    }
    
    public void removeHelper(String key, TrieNode n){
        checkKey(key);
        char firstChar = key.charAt(0);
        TrieNode child = n.getChild(firstChar);
        if(key.length() == 1 && child != null && child.endsPath == true){
            child.endsPath = false;
        }
        else if(child != null && key.length()>1){
            removeHelper(key.substring(1), child);
        }
    }
    ...
    public void match(String pattern) {
        ...
        // TODO implement this method
        matchHelper(pattern,this,"");
    }
    
    public void matchHelper(String pattern, TrieNode n,String k){
        if(n == null)
            return;
        if (pattern.length()>0){
            ...
            if(firstChar == '*'){
                ...
                for(int l = 0; l<n.children.length; l++){
                    if(n.children[l]== null)
                        continue;
                    TrieNode child = n.children[l];
                    char ch = indexToChar(l);
                    matchHelper(pattern.substring(1),child,k+ch);
                }
            }
            
            else{
                TrieNode child = n.getChild(firstChar);
                matchHelper(pattern.substring(1),child,k+firstChar);
            }
        }
        
        else if(n.endsPath == true && pattern.length()==0){
            System.out.println(k);
        }
    }
